...
> **Status: SHIPPED (MVP)** on `feature/phase2-eval-harness` (410/410 tests). Built `evals/`: a **deterministic scorer** (`score.sh` — CI-tested L17, 8 assertions: cost summing, findings, retries, wall extraction; no model calls), a **held-out oracle** model (grading tests live in `tasks/<t>/oracle/`, applied post-run against a copy the pipeline never saw — verified pass@1 discriminates fix vs no-fix and is held out of `repo/`), an **opt-in real-Claude runner** (`run.sh`, full-pipeline-capable, `--max-budget-usd` cost-cap, baseline + claudehut modes), and `compare.sh` (A/B). **One real baseline row produced** (`results/baseline.jsonl`: trivial-sum-bug, real claude, pass@1=1, cost \$0.14, 13s) — the harness is proven end-to-end, not a never-run artifact. Seed fixture: `trivial-sum-bug` (trivial class — highest router signal). **Cost caveat (verified):** scorer sums main-session `total_cost_usd` (orchestrator + in-process Task subagents) + Σ `.claudehut/logs/*.cost`, but Path-B build workers don't yet emit `.cost` (Phase-5 telemetry) → claudehut-mode cost is undercounted by build-worker spend; baseline mode is exact. **Remaining (opt-in / next):** the full Spring suite (Kafka+DLT, Flyway, reactive handler, one large feature) and a real **claudehut-mode** row — the latter also exercises the unverified headless-orchestrator-prose dispatch seam (Phase-0 §2 residual), so it's both the A/B data and that seam's first real test.
