...
**Remaining honest residuals (design-inherent, not incomplete work):** (a) 1.4's retry threshold is enforced as a *deterministic surfacing*, not a hard block — blocking mid-loop is riskier than surfacing; a true hard gate belongs with the Phase-3 router. (b) `claudehut-finish` still references a legacy `state/tasks/<id>` archive model that the artifact-derived design doesn't populate — the `mv` is now tolerant so finish works, but "what finish should archive" (likely the `.claudehut/specs|plans|findings` set) is a separate design question for a later phase.
