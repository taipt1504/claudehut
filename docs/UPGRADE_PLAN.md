...
> **Status: SHIPPED** on `feature/phase1-robustness` (391/391 tests). Headline discovery: **`bin/claudehut-state` was entirely broken** — it sourced a nonexistent `scripts/hooks/lib/state.sh` and exited 1 on *every* call, silently killing every `claudehut-state {phase,retries,stack,config,docs}` agent-prose invocation. Fixed the lib path (bigger than 6.3's dot-prefix) + added an L2.1b "bin actually runs" test (it had only ever been `bash -n`'d). Done: 1.1 stop bounded-escape via `stop_hook_active` (platform field) + manual escape hatch; 1.2 `has_learnings` jq (pretty-JSON safe); 1.3 deterministic migration gate wired into pre-tool.sh (validates `tool_input.content` via temp file, deny on exit 1, allow on exit 2, no worker bypass) + migration-validator agent demoted to Loop-time review + stale refs fixed; 1.4 `loop_max_retries` now *readable* (`claudehut-state config`) and the verifier is instructed to read it (honest framing: readable + instructed, **not enforced** — LLM-prose, same class as Phase-0 §2 residual); 1.5 destructive block de-theatered (honest "speed-bump, not a sandbox" + golden updated); 1.6 bin lib-path + stack dot-prefix + help text; 1.8 atomic `integrations.json` (same-dir tmp+mv); 1.9 confirmed no-op (docs already correctly state CLAUDE.md reaches custom subagents, per B.2). **Deferred: 1.7** (`active-task.json` pointer) — session-start runs every session, not at task start, so a writer there is semantically wrong; readers already fall back safely. Revisit when a real task-start hook exists (e.g. with the Phase-3 router).
