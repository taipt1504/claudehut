...
> **Remaining → 3.2 (opt-in, costs $):** re-run the eval on `trivial-sum-bug` in claudehut mode and confirm routed `quick` fixes the bug at **materially lower cost** than the $1.24 full-ceremony budget-kill (and far below `full`'s tax). This also first-exercises the headless route-step dispatch + quick-inline-build seam end-to-end (still unproven without a real run, like the Phase-2 §2 residual).
