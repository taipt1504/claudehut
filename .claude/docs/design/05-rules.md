...
| `framework/` | `spring-mvc`, `webflux`, `jpa`, `r2dbc`, `kafka-consumer`, `kafka-producer`, `rabbitmq`, `nats`, `redis`, `jackson`, `mapstruct`, `flyway-naming`, `migration-safety`, `lombok-annotations`, `lombok-builder`, `lombok-jpa-safety` | `**/*Controller.java`, `**/*Handler.java`, `**/*Entity.java`, `**/*Listener*.java`, `**/db/migration/V*.sql`, … | per-framework standards; **stack-gated** by `stack:` (e.g. `web=webflux`, `orm=r2dbc`, `messaging=kafka`) |
| `performance/` | `n-plus-one`, `indexing`, `connection-pool`, `caching`, `backpressure` | `**/*Repository.java`, `**/db/migration/V*.sql`, `**/application*.yml`, `**/*Handler.java` | N+1, index design, pool sizing, cache TTL, reactive backpressure |
| `security/` | `spring-security`, `owasp-top10`, `input-validation`, `deserialization`, `secret-mgmt`, `actuator` | `**/SecurityConfig*.java`, `**/*Controller.java`, `**/*.java`, `**/application*.yml` | deny-by-default authz, OWASP Top 10, validation, safe deserialization, secret mgmt, actuator exposure |
