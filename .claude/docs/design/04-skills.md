...
6. **`context: fork`** for skills whose real work belongs in a single subagent — routes the turn into the matching agent ([03](./03-agents.md)). Skills that must **spawn multiple subagents** run **inline on the main thread**, because a forked subagent cannot spawn subagents. The two inline-multi-subagent skills are **`brainstorm`** (spawns `claudehut-explorer` → `claudehut-reuse-scanner` → `claudehut-brainstormer` in sequence) and **`review`** (spawns the five auditors in parallel). The `context: fork` skills are **`write-plan`** (→ `claudehut-planner`) and **`capture-learnings`** (→ `claudehut-learner`).
...
| **Phase / Enforcement** | Drive and gate each phase | auto-trigger by `description`; Iron-Law bodies; `context: fork` for single-subagent phases; inline for multi-subagent phases |
...
- **Trigger:** `/claudehut:init`; **or** auto-suggested by `bootstrap.sh` (which sets `initialUserMessage` when `.claude/claudehut/` is absent).
...
- **Runs INLINE on the main thread** — dispatches three subagents in sequence via the Agent tool:
  ...
  2. [`claudehut-reuse-scanner`](./03-agents.md#claudehut-reuse-scanner) — reuse-scan step: produces `.claude/claudehut/reuse-scan-<task>.md` (adopt/extend/none + justification); runs `claudehut-state set-reuse-scan`.
...
- **Cannot use `context: fork`** because a forked subagent cannot itself spawn subagents.
...
- **I/O:** out: the implementation spec at `.claude/claudehut/specs/NNNN-<slug>.md` (context · chosen approach · acceptance criteria · enforcement manifest · rejected alternatives — it **subsumes the old ADR**). The agent runs `claudehut-state set-spec <path>` after writing. **REQUIRED NEXT:** `claudehut:write-plan`.

### write-plan
- **Phase:** Plan. **Trigger:** after the spec is written.
- **Native:** `context: fork` → [`claudehut-planner`](./03-agents.md#claudehut-planner).
- **I/O:** out: plan file. On completion the agent runs `claudehut-state set-phase plan --plan <path>` (this is what unlocks the write gate, [06](./06-hooks.md)). **REQUIRED NEXT:** `claudehut:implement`.
...
- **What it does (the Review loop):** dispatches the five auditor agents; each enforces one lens against the enforcement set + project rules/memory and returns its **outstanding items**. The skill merges them via `claudehut-state set-outstanding`.
...
- **Native:** skill (no `context: fork`); the `Stop` hook blocks turn end until `state.json.review=pass` (or `capped`). **Pairs with:** the `gate-done.sh` completion gate ([06](./06-hooks.md)). **REQUIRED NEXT:** `claudehut:capture-learnings`.

### capture-learnings
- **Phase:** Learn. **Trigger:** after a successful review; end of task; "capture / record / learnings".
- **Native:** `context: fork` → [`claudehut-learner`](./03-agents.md#claudehut-learner).
...
- **Process:** extract candidate learnings → dedup against `learnings.jsonl` → append/merge → update `reuse-index.json` → append narrative to native auto-memory.
...
flowchart LR
    ...
    SP --> WP[write-plan]
    WP -->|fork: claudehut-planner| IM[implement]
    IM -->|rules + references| IM
    IM --> RV[review]
    RV -->|inline: 5 auditors| CL[capture-learnings]
    CL -->|fork: claudehut-learner| done[task complete]
```
...
- **Inline vs. fork:** `brainstorm` and `review` run inline (multi-subagent); `write-plan` and `capture-learnings` use `context: fork` (single-subagent); `write-spec` and `implement` run on the main thread directly.
