...
> **v0.3.0** · a Claude Code plugin for **Java / Spring Boot backend engineers**.

ClaudeHut turns a single task description into a disciplined, six-phase engineering loop — and **enforces**
it with native Claude Code mechanisms (hooks, skills, subagents, path-scoped rules) rather than relying on
the model to remember:

```
            ┌─────────────────────────────────────────────────────────────┐
  init  →   │  Brainstorm → Spec → Plan → Implement → Review → Learn        │
(pre-index) └─────────────────────────────────────────────────────────────┘
              reuse-first      test-first        evidence-first   reinforced
```
...
<describe your task>     # ClaudeHut drives Brainstorm → … → Learn automatically
```
...
- **Write gate** (`PreToolUse`): no new production code until a reuse-scan artifact, a spec, **and** a plan
  exist (think-and-reuse before you build). Test paths (`*Test.java`, `*IT.java`, `*/test/*`) are always
  allowed so the RED test can come first. The gate also verifies the named artifacts actually exist at
  canonical paths — a flag alone won't unlock it.
- **Completion gate** (`Stop`): you can't claim "done" until Review reports zero outstanding items and the
  Learn pass has run (honors the native consecutive-Stop cap). Sessions that never engaged the workflow
  aren't blocked.
- **Iron-Law skills** order actions within a turn — reuse-first (Brainstorm), test-first (`implement`'s
...
"${CLAUDE_PLUGIN_ROOT}/bin/claudehut-state" --session "$CLAUDE_SESSION_ID" set-phase implement  # jump phases for a trivial task
```
...
- **Skills** (`skills/`) — 8 total: orchestrator (`claudehut-workflow`) + indexer (`claudehut-init`) + one
  per phase (`brainstorm`, `write-spec`, `write-plan`, `implement`, `review`, `capture-learnings`). The
  `implement` skill carries the TDD Iron Law and the tech-stack playbooks.
...
- **CLI tools** (`bin/`) — `claudehut-init` (deterministic stack-detect + project-plane generator),
  `claudehut-state` (the sole writer of per-session phase state), and `kafka-mcp` (an optional, documented
  **stub**).
