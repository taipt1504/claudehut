...
### 3. Stack-Aware Rule Auto-Loading

ClaudeHut detects your stack at `SessionStart` (Spring MVC vs WebFlux, JPA vs R2DBC, MapStruct vs manual, Jackson, Kafka/RabbitMQ/NATS) and persists to `.claudehut/memory/stack-signals.json`. When you edit a file, the `PreToolUse` hook auto-loads matching rules via `rules-index.json`:

```jsonc
{"glob": "**/*Controller.java", "rule": "rules/framework/spring-mvc.md", "if_stack": "web=mvc"}
{"glob": "**/*Handler.java",    "rule": "rules/framework/webflux.md",   "if_stack": "web=webflux"}
{"glob": "**/*Repository.java", "rule": "rules/framework/r2dbc.md",     "if_stack": "orm=r2dbc"}
{"glob": "**/*Mapper.java",     "rule": "rules/framework/mapstruct.md"}
{"glob": "**/db/migration/V*.sql", "rule": "rules/framework/migration-safety.md"}
```

42 rules total, organized in 6 categories: coding, architecture, testing, security, performance, framework-specific. Each rule is a markdown file with DO/DON'T + examples + cross-references. Project can override any rule by placing a file under `.claudehut/rules/`.
...
<repo>/.claudehut/                    ← Project tier (committed, team-shared)
└── memory/
    ├── conventions.md
...
│   │   ├── index.md                  ← reusable impl map
│   │   ├── stack-signals.json        ← detected stack
...
A: Yes — drop files under `.claudehut/rules/`. Project-local rules override plugin defaults. See `rules/` directory + `rules/rules-index.json` for the auto-load mapping.
